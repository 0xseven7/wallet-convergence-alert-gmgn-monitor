(function () {
  ...
  function normalizeQuickAddChain(chain) {
    return CHAIN_MAP[String(chain || '').trim().toLowerCase()] || '';
  }
  ...
  function parseGmgnTokenPageUrl(rawUrl) {
    ...
    const allParts = [...parts, ...hashParts];
    let chain = '';
    let ca = '';
    ...
    if (!chain || !ca) {
      const tokenRoute = parseTokenRouteParts(allParts);
      chain = chain || tokenRoute.chain;
      ca = ca || tokenRoute.ca;
    }

    if (!chain || !ca) {
      const queryChain = parsed.searchParams.get('chain')
        || parsed.searchParams.get('network')
        || parsed.searchParams.get('chainId')
        || hashParams.get('chain')
        || hashParams.get('network')
        || hashParams.get('chainId');
      const queryAddress = parsed.searchParams.get('ca')
        || parsed.searchParams.get('contractAddress')
        || parsed.searchParams.get('contract_address')
        || parsed.searchParams.get('address')
        || parsed.searchParams.get('token')
        || parsed.searchParams.get('tokenAddress')
        || parsed.searchParams.get('mint')
        || hashParams.get('ca')
        || hashParams.get('contractAddress')
        || hashParams.get('contract_address')
        || hashParams.get('address')
        || hashParams.get('token')
        || hashParams.get('tokenAddress')
        || hashParams.get('mint');
      chain = chain || pickFirstChain([queryChain, ...allParts]);
      ca = ca || pickFirstTokenAddress([queryAddress, ...allParts]);
    }
