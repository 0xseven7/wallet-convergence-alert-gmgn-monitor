...
一个同时支持 **xxyy.io** 和 **gmgn.ai** 的 Chrome 扩展，用来监控钱包追踪页面里的交易流，做聚合买入提醒，并为 GMGN 增加一套独立的 Twitter 语音提示、follow 信息流语音特别关注、Relay 跳转和页面清理能力。
...
- `主屏`：你平时用来查看代币详情、打开外链、继续深挖页面的 Chrome profile / 窗口，使用独立的轻量主屏扩展。

当前设置方式：

1. 在主屏 profile 里安装主屏轻量扩展
2. 在监控屏 profile 里安装 monitor 扩展
3. 两边都配置到同一个 Relay Base URL
4. 监控屏继续打开 `gmgn.ai/follow`，主屏负责承接跳转

当前行为：

- 在监控窗口里点击代币、外链或由监控页触发的跳转时，monitor 会把打开请求交给 Relay，再由主屏扩展在主屏 profile 打开
- 监控窗口会尽量继续停留在 `follow` 页面，避免监控页被跳转带走
- monitor 设置页提供“将当前窗口设为监控屏”入口，只绑定设置页所在 Chrome 窗口中的 GMGN Follow 标签；独立的 FOMO 小窗口不会被绑定
